...
export default function PolicyContent({ content }: { content: string }) {
  ...
  const sections = useMemo<PolicySection[]>(() => {
    ...
    sectionStartIndices.forEach((startIndex, idx) => {
      ...
      const heading = parseSectionHeading(blocks[startIndex].split("\n")[0]);
  ...
  const titleBlock = blocks.find((block) => block.startsWith("PRIVACY POLICY"));
  const summaryIndex = blocks.findIndex((block) => block.startsWith("SUMMARY OF KEY POINTS"));
  ...
  const summaryParagraphs = useMemo(() => {
    ...
    const summaryEndCandidates = [tocIndex, firstSectionIndex].filter((index) => index > summaryIndex);
    const summaryEnd =
      summaryEndCandidates.length > 0 ? Math.min(...summaryEndCandidates) : blocks.length;

    return blocks
      .slice(summaryIndex + 1, summaryEnd)
      .map((block) => block.trim())
      .filter(Boolean);
  }, [blocks, summaryIndex, tocIndex, firstSectionIndex]);

  const introParagraphs = useMemo(() => {
    const titleIndex = blocks.findIndex((block) => block.startsWith("PRIVACY POLICY"));
    const introStart = titleIndex === -1 ? 0 : titleIndex + 1;
    const introEndCandidates = [summaryIndex, tocIndex, firstSectionIndex].filter((index) => index >= introStart);
      ...
      {summaryIndex !== -1 && (
        <section className="mb-10">
          ...
          {summaryParagraphs.map((line, index) => (
            <p key={index} className="text-gray-700 leading-relaxed mb-3 [text-align:justify]">
              {line}
            </p>
          ))}
        </section>
